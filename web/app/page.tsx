...
function Program({ program, index, className = '' }: { program: Landing['programs'][number]; index: number; className?: string }) {
  return (
    <article className={`${styles.program} ${className}`} data-reveal>
  ...
  return (
    <main id="main-content">
      ...
      <section className={`section container ${styles.programsSection}`} id="what-we-do" aria-labelledby="programs-heading">
        <div className={styles.sectionHead} data-reveal><p className="eyebrow">02 / What we do</p><h2 id="programs-heading">{d.programsHeadline}</h2></div>
      ...
      <section className={`section container ${styles.room}`} id="in-the-room" aria-labelledby="room-heading">
        <div className={styles.roomIntro} data-reveal><div><p className="eyebrow">04 / In the room</p><h2 id="room-heading">{d.roomHeadline}</h2></div><p>From first introductions to shared ambitions. A few moments with our community.</p></div>
