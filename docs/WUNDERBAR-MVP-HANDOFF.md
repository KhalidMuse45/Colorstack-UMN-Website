...
The user approved building the MVP after reviewing the implementation plan. The first working application is in `wunderbar/`; the ColorStack application and its deployment remain separate.
...
**Run/review:** follow `wunderbar/README.md`. The main application routes are `/`, `/practice`, `/login`, and `/privacy`. Workspace navigation uses URL fragments, with session/story dialogs inside the workspace. The server API is `/api/workspace`; the separately authenticated email worker is `/api/reminders`.
